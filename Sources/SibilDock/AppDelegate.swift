...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // collectionBehavior/level tricks (omitting `.fullScreenAuxiliary`,
        // dropping window level, disabling isFloatingPanel) turned out not
        // to actually exclude an accessory app's panels from a full-screen
        // Space on this system — verified empirically, still showing with
        // every combination. So instead the panel always uses the original
        // always-on-top setup, and hiding is done explicitly: ordered out
        // when the active Space is a full-screen one and the setting is on,
        // ordered back in otherwise. NSWorkspace has no "did enter/exit
        // full screen" notification for other apps, but activeSpaceDidChange
        // fires on every Space switch, full-screen included.
        NSWorkspace.shared.notificationCenter.addObserver(
            self,
            selector: #selector(updateFullScreenVisibility),
    ...
    @objc private func updateFullScreenVisibility() {
        let shouldHide = DockSettings.shared.hidesDuringFullScreen && FullScreenSpaceDetector.isCurrentSpaceFullScreen()
        for panel in [floatingPanel as NSPanel?, edgePanel as NSPanel?].compactMap({ $0 }) {
            if shouldHide {
                panel.orderOut(nil)
            } else {
...
/// DraggableHostingView. Follows the user across every Space, full screen
/// included; whether it's actually visible during full screen is handled
/// separately by AppDelegate explicitly ordering it in/out based on the
/// hidesDuringFullScreen setting (see updateFullScreenVisibility).
